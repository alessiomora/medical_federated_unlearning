...
def main(cfg: DictConfig) -> None:
    ...
    _, _, ds_test, _, _, _ = utility.get_test_and_train_dataset() # get ds_test_union
    ...
    if restart_training:
        if retraining:
            ...
            print(f"[{algorithm} baseline] Searching for checkpoints...")
            model_checkpoint_base_dir = os.path.join(model_checkpoint_dir,
                                            "checkpoints")
            last_round = find_last_checkpoint(model_checkpoint_base_dir)
            if last_round > 0:
                print(f"[{algorithm} baseline] Found previous checkpoint.")
                model_checkpoint_dir_to_load = os.path.join(model_checkpoint_base_dir,
                                                        f"R_{last_round}")
                server_model.load_weights(model_checkpoint_dir_to_load)
                resumed_round = last_round
    ...
    if not early_stop_recovery:
        ...
        for r in range(resumed_round + 1, resumed_round + total_rounds + 1):
            ...
            unl_client_examples = local_n_examples[unlearned_cid]
            ...
            if r == SAVE_ROUND_CLIENTS+1 and algorithm in ["pseudo_gradient_ascent", "pseudo_gradient_ascent_single"]:
                ...
                for u in unlearned_cid:
                    local_samples = local_n_examples[u]
                    print("Local samples: ", local_samples)

                    client_model = server_model
                    client_model.set_weights(global_weights)
                    client_update(u, client_model, epochs=epochs_unlearning, learning_rate=learning_rate, sites=sites, local_batch_size=local_batch_size, model_checkpoint_dir=model_checkpoint_dir, t=r, retraining=retraining)
